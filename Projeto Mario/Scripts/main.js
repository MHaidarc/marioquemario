...
var Level = Base.extend({
	...
	playSound: function(label) {
		var audio = new Audio(sounds[label]);
		audio.play();
 ...
 * -------------------------------------------
 */
var Freddie = Enemy.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setSize(117, 188);
		this.setSpeed(constants.ballmonster_v*2);
		this.death_mode = death_modes.shell;
		this.deathCount = 0;
	},
	setVelocity: function(vx, vy) {
		this._super(vx, vy);
		...
		if(this.direction === directions.left) {
			if(!this.setupFrames(6, 2, false, 'LeftWalk'))
				this.setImage(images.Freddie, 0, 8);
		} else {
			if(!this.setupFrames(6, 2, true, 'RightWalk'))
				this.setImage(images.Freddie, 0, 8);
		}
	...
	death: function() {
		if(this.death_mode === death_modes.shell)
			return --this.deathCount;
	...
	die: function() {
		...
		if(this.death_mode === death_modes.normal) {
			...
			this.level.playSound('shell');
			this.setImage(images.Freddie, 0, this.direction === directions.right ? 228 : 188);
			this.deathFrames = Math.floor(250 / constants.interval);
			this.deathDir = 1;
			this.deathStep = Math.ceil(150 / this.deathFrames);
		}
